...
- 在玩家名字左侧提供“强化榜”按钮；打开后可选择“填单榜”或“挂单榜”。
- 两个榜单均可按工时费/h、风控工时费/h 或经验/h 排序，并会在传统强化、普通强化后精炼、贤者之镜路线中选取时薪最高的可行方案。
- 填单榜使用最高收购价（Bid）作为成品出售价；挂单榜使用最低挂单价（Ask）作为成品出售价；均按 5% 交易税计算税后收入，材料和保护道具成本沿用原有市场价格逻辑。
- 贤者之镜路线会把镜子阶段按 100% 成功的确定动作处理，并在榜单中列出所需的精确前置成品、镜子数量及其期望耗时；风控工时会汇总前置强化的随机成本。
- 点击榜单中的物品可跳转到对应 +N 市场；脚本会等待该商品订单簿刷新后只更新对应的榜单条目。
- 强化榜入口不干预游戏原生标签页，也不与其他脚本的标签切换逻辑耦合。
...
1. 进入游戏后，在玩家名字左侧点击“强化榜”。
2. 选择“填单榜”或“挂单榜”。首次打开会按当前 Buff、装备和市场快照进行完整本地计算；完成后会显示本次计算耗时。
